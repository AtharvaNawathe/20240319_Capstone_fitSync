...
const moveMealToHistory = async (req, res) => {
    try {
        ...
        const mealTypeEntryIndex = mealPlan.meal_plan.findIndex(entry => entry.meal_type === meal_type);

        if (mealTypeEntryIndex === -1) {
            return res.status(404).json({ message: 'No meal found for the provided type' });
        }
        ...
        res.status(200).json({ message: 'Meal data moved to history and entire meal type\'s data removed from meal plan successfully' });
    } catch (error) {
        console.error(error);
